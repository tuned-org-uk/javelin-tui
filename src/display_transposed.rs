...
/// Rows for transposed mode: one row per feature (col_*), scrolling over rows.
///
/// For each feature:
/// - first cell: feature name,
/// - next cells: values for visible rows,
/// - last two: avg/std over all rows for that feature.
fn render_rows_transposed<'a>(
    batch: &'a RecordBatch,
    row_window: &'a [usize],
    all_cols: &'a [usize],
) -> Vec<Row<'a>> {
    let mut out = Vec::with_capacity(all_cols.len());
